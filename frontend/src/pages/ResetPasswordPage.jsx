    ...
    <main className="grid min-h-screen place-items-center bg-neighbor-mist px-4">
      <form onSubmit={onSubmit} className="card w-full max-w-md p-7">
        <div className="text-center">
          ...
          <h1 className="mt-3 text-2xl font-black text-neighbor-navy">Crear nueva contrasena</h1>
        ...
        <div className="mt-5 space-y-4">
          <input className="input" type="password" placeholder="Nueva contrasena" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={6} />
          <input className="input" type="password" placeholder="Confirmar contrasena" value={confirm} onChange={(e) => setConfirm(e.target.value)} required minLength={6} />
          <button className="btn-primary w-full" disabled={loading || !token}>{loading ? 'Guardando...' : 'Actualizar contrasena'}</button>
        </div>
